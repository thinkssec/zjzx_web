...
<ul class="nav nav-tabs">
    <li class="active"><a href="">数据升级管理</a></li>
    <%--<shiro:hasPermission name="cbgl:rgcblist2"><li><a href="${ctx}/cbgl/rgcblist">人工成本发布2</a></li></shiro:hasPermission>--%>
</ul>
<form:form id="searchForm" action="${ctx}/bbgl/sjlist" method="post" class="breadcrumb form-search" onsubmit="loading('正在载入，请稍等...');">
...
<script type="text/javascript">
    ...
    var dgOptions = {
        ...
        border: true,
        multiSort: true,
        url: '${ctx}/bbgl/sjbbList',
        columns: [[
            {field: 'ID',  hidden: false, checkbox: true,align:'center'}
            , {field: 'BBH', title: '版本号', width: 40,align:'center',editor: "text"}
            /*, {field: 'TYPEID', title: '数据类型', width: 40,align:'center',editor: {
                type: 'combobox',
                editable: "false",
                options: {
                    valueField: 'TYPEID',
            ...
            , {field: 'CZZ', title: '操作', width: 40,align:'center',
                formatter: function(value,row,index){
                    if(row.ID&&row.ID!=''&&row.STATUS!='1')
                        return "<a href='#' name='opera' class='easyui-linkbutton' onclick='document.location=\"/static/ckfinder/ckfinderSj.jsp?ID="+row.ID+"&BBH="+row.BBH+"\"'>上传</a>";
                    else
                        return "";
            }}
        ]],
    ...
    function xf(){
        ...
        $('#searchForm').attr("action",'${ctx}/bbgl/bbsjxf').submit();
    ...
    function delZh(){
        ...
        $('#hid_c2').val(JSON.stringify(ttt));
        $('#searchForm').attr('action','${ctx}/bbgl/sjbbdel').submit();
    }
</script>
</body>
</html>